            ...
            if bloodPressure.category == .stage2Hypertension {
                alerts.append(
                    HealthAlert(
                        ...
                        title: "血圧注意",
                        description:
                            "血圧が高めです（\(Int(bloodPressure.systolic))/\(Int(bloodPressure.diastolic))mmHg）。生活習慣の見直しをお考えください。",
                        severity: .critical,
                        actionRequired: true,
                        recommendations: [
                            "塩分摂取を控える",
        ...
        if data.sleep.sleepEfficiency < 0.7 {
            alerts.append(
                HealthAlert(
                    ...
                    title: "睡眠の質注意",
                    description:
                        "睡眠効率が低下しています（\(String(format: "%.0f", data.sleep.sleepEfficiency * 100))%）。睡眠環境を見直してみませんか？",
                    severity: .warning,
                    recommendations: [
                        "就寝前のスクリーン時間を減らす",
                        "寝室の温度を調整する",
        ...
        if data.sleep.sleepEfficiency < 0.8 && data.sleep.sleepEfficiency >= 0.7 {
            alerts.append(
                HealthAlert(
                    ...
                    title: "🌙 睡眠の質向上のヒント",
                    description:
                        "睡眠効率が\(String(format: "%.0f", data.sleep.sleepEfficiency * 100))%です。少しの工夫でさらに改善できそうです。",
                    severity: .info,
                    recommendations: [
                        "就寝前1時間はスマホを見ない",
                        "寝室の温度を18-20度に保つ",
