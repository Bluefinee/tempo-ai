...
Tempo AIの健康メトリクス体系は、**自律神経バランス**と**サーカディアンリズム**を核心として設計されています。

| 核心概念 | 説明 | 対応メトリクス |
|---------|------|--------------|
| 自律神経バランス | 交感神経と副交感神経のバランス状態 | HRV、ストレスグラフ |
...
| メトリクス | スコア | 表示内容 | アイコン |
| ---------- | ------ | -------- | -------- |
| 睡眠 | 0-100 | 昨夜の睡眠時間、深い睡眠の割合、7日平均 | 三日月+Z |
| HRV | 0-100 | 今朝の値、7日平均との差、トレンド | 心臓+波形 |
| リズム | 0-100 | 平均就寝・起床時刻、リズム安定度 | 時計+サイクル |
| 活動量 | 0-100 | 今日の歩数、7日平均、今週の運動回数 | 歩く人 |

※ 詳細なスコア算出アルゴリズムは [metrics-algorithm-spec.md](./metrics-algorithm-spec.md) を参照

#### 4.2.2 ストレスグラフ

メトリクスカードとは別に、24時間の自律神経バランスを可視化するグラフを表示。

| 項目 | 内容 |
| ---- | ---- |
| 時間範囲 | 24時間（過去24時間または当日） |
| データ間隔 | 5分間隔 |
| Y軸 | -100（ストレス）〜 +100（リラックス） |
| 色 | グラデーション（緑=リラックス、黄=中間、赤=ストレス） |
...
| 画面                             | 内容                                                        |
| -------------------------------- | ----------------------------------------------------------- |
| 睡眠詳細                         | 睡眠時間、深い睡眠・REM睡眠の割合、週間トレンド、スコア内訳 |
| HRV詳細                          | 今朝・7日平均・30日平均、週間トレンドグラフ、ベースライン比較 |
| リズム詳細                       | 24時間サークル図、週間トレンドグラフ、安定度スコア、ヒント  |
| 活動量詳細                       | 歩数、運動時間、消費カロリー、週間トレンド、スコア内訳      |
| ストレスグラフ詳細               | 24時間グラフ、ピーク時間、リラックス時間、インサイト        |
| 環境詳細                         | 気温、湿度、気圧、AQI、UV指数                               |
...
| メトリクス | 主要コンポーネント |
| ---------- | ------------------ |
| 睡眠 | 睡眠時間(35) + 深い睡眠(25) + REM睡眠(15) + 睡眠効率(15) + 就寝タイミング(10) |
| HRV | 個人ベースライン比較(50) + 7日トレンド(25) + 安静時心拍数(25) |
| リズム | 就寝時刻安定度(35) + 起床時刻安定度(35) + 週末シフト(20) + 理想時間帯(10) |
| 活動量 | 歩数(40) + アクティブ時間(30) + 座りっぱなし回避(20) + 運動完了(10) |
