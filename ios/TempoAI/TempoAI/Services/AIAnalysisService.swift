...
class AIAnalysisService: ObservableObject {
    ...
    private func generateActionableRecommendations(
        ...
        recommendations.append(contentsOf: aiInsights.recommendations.map { aiRec in
            ActionableRecommendation(
        ...
        if weather.temperature < 10 || weather.temperature > 30 {
            ...
            recommendations.append(ActionableRecommendation(
                ...
                description: localizationManager.currentLanguage == .japanese ?
                    ...
                    (isHot ? "Temperature is high. Consider indoor exercise or cooler time periods" : "Temperature is low. Consider indoor exercise and proper warm-up"),
                priority: .medium,
                actionableSteps: [
                    localizationManager.currentLanguage == .japanese ? (isHot ? "早朝または夕方の運動" : "十分なウォームアップ") : (isHot ? "Exercise in early morning or evening" : "Adequate warm-up period"),
                    localizationManager.currentLanguage == .japanese ? "水分補給の意識" : "Stay hydrated"
                ],
                estimatedBenefit: localizationManager.currentLanguage == .japanese ? "安全で効果的な運動" : "Safe and effective exercise",
                source: .environmental
            ))
        }
        ...
        if localAnalysis.overallScore < 60 {
            recommendations.append(QuickRecommendation(
                text: localizationManager.currentLanguage == .japanese ? 
                    "今日は休息を優先してみませんか？" : 
                    "Consider prioritizing rest today",
                category: .lifestyle
            ))
        } else if localAnalysis.overallScore > 80 {
            recommendations.append(QuickRecommendation(
                text: localizationManager.currentLanguage == .japanese ? 
                    "素晴らしい健康状態です！この調子で続けましょう" : 
        ...
        for trend in localAnalysis.trends {
            if trend.direction == .declining && trend.significance > 0.7 {
                let trendRec = QuickRecommendation(
                    text: localizationManager.currentLanguage == .japanese ? 
                        "\(trend.metric)の改善に注目してみませんか？" : 
                        "Consider focusing on improving your \(trend.metric)",
    ...
    private func generatePrimaryInsight(
        ...
        if score >= 80 {
            return localizationManager.currentLanguage == .japanese ? 
                "健康状態は良好です。この調子で継続しましょう" : 
                "Your health status is excellent. Keep up the great work"
        } else if score >= 60 {
            let improvingTrends = trends.filter { $0.direction == .improving }
            if !improvingTrends.isEmpty {
                return localizationManager.currentLanguage == .japanese ? 
                    "改善傾向が見られます。良い方向に向かっています" : 
                    "Positive trends detected. You're moving in the right direction"
            } else {
                return localizationManager.currentLanguage == .japanese ? 
                    "いくつかの改善点があります。一歩ずつ前進しましょう" : 
                    "Some areas for improvement identified. Let's take it step by step"
            }
        } else {
            return localizationManager.currentLanguage == .japanese ? 
                "今日は休息を重視してみませんか？" : 
                "Consider prioritizing rest and recovery today"
        }
    }
}
