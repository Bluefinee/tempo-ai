...
import { Pause, Play, Wind, X } from "lucide-react-native";
...
// Breathe画面専用カラー定数（themeから参照）
const BREATHE_COLORS = {
	background: colors.deepNavy, // #0F172A
	inhale: colors.indigo[400], // #818CF8
	hold: "#A78BFA", // violet-400 (themeに未定義のためここで定義)
	exhale: colors.indigo[500], // #6366F1
} as const;
...
const BreatheScreen = (): React.ReactElement => {
	...
	const getPhaseColor = (): string => {
		switch (phase) {
			case "inhale":
				return BREATHE_COLORS.inhale;
			case "hold":
				return BREATHE_COLORS.hold;
			case "exhale":
				return BREATHE_COLORS.exhale;
			default:
				return BREATHE_COLORS.exhale;
		}
	};
...
const styles = StyleSheet.create({
	container: {
		backgroundColor: BREATHE_COLORS.background,
	},
	backgroundGradient: {
		position: "absolute",
		top: 0,
