...
import { TOOLTIP_AUTO_HIDE_DELAY } from "../constants/rhythmConstants";
import { t } from "../i18n";
...
export const RhythmInteractiveChart = ({
	...
	const xAxisLabels = useMemo(() => {
		if (data.length === 0) return [];
		const labels = [
			t("chart.timeAxis.6am"),
			t("chart.timeAxis.12pm"),
			t("chart.timeAxis.6pm"),
			t("chart.timeAxis.12am"),
		];
		const minHour = Math.min(...data.map((d) => d.hour));
		const maxHour = Math.max(...data.map((d) => d.hour));
		const hourRange = maxHour - minHour || 1;
	...
	const formatCurrentTime = () => {
		...
		const period = hours >= 12 ? t("common.time.pm") : t("common.time.am");
					...
					>
						<View style={[styles.badgeInner, styles.peakBadge]}>
							<Text style={styles.peakBadgeText}>
								{t("screen.rhythm.labels.peakFocus")}
							</Text>
						</View>
					</View>
				)}
				...
				{annotations.dip && !isDragging && (
					...
					>
						<View style={[styles.badgeInner, styles.dipBadge]}>
							<Text style={styles.dipBadgeText}>
								{t("screen.rhythm.labels.afternoonDip")}
							</Text>
						</View>
					</View>
				)}
