...
class AIUsageAnalyzer {
    ...
    func analyzeUsagePatterns(_ records: [AIRequestRecord]) -> UsagePatternAnalysis {
        ...
        let timeOfDayDistribution = analyzeTimeOfDayDistribution(records)
        let requestTypeFrequency = analyzeRequestTypeFrequency(records)
        let costPerSatisfaction = analyzeCostEffectiveness(records)
        let unusualPatterns = detectUnusualPatterns(records)

        return UsagePatternAnalysis(
            timeOfDayDistribution: timeOfDayDistribution,
            requestTypeFrequency: requestTypeFrequency,
            costEffectiveness: costPerSatisfaction,
            unusualPatterns: unusualPatterns,
            recommendations: generateOptimizationRecommendations(records)
        )
    }

    private func analyzeTimeOfDayDistribution(_ records: [AIRequestRecord]) -> [Int: Double] {
        var hourCounts: [Int: Int] = [:]
        ...
        return hourCounts.mapValues { Double($0) / Double(total) }
    }

    private func analyzeRequestTypeFrequency(_ records: [AIRequestRecord]) -> [AnalysisRequestType: Double] {
        var typeCounts: [AnalysisRequestType: Int] = [:]

        for record in records {
            typeCounts[record.type, default: 0] += 1
        }

        let total = records.count
        return typeCounts.mapValues { Double($0) / Double(total) }
    }

    private func analyzeCostEffectiveness(_ records: [AIRequestRecord]) -> Double {
        let recordsWithSatisfaction = records.filter { $0.userSatisfaction != nil }
        guard !recordsWithSatisfaction.isEmpty else { return 0.5 }

        let totalSatisfaction = recordsWithSatisfaction.reduce(0) { $0 + ($1.userSatisfaction ?? 0) }
        let totalCost = recordsWithSatisfaction.reduce(0) { $0 + $1.cost }

        return totalSatisfaction / (totalCost * 100)  // Satisfaction per cent
    }

    private func detectUnusualPatterns(_ records: [AIRequestRecord]) -> [UsageAnomalyPattern] {
        var patterns: [UsageAnomalyPattern] = []

        // Detect burst patterns (many requests in short time)
        let sortedRecords = records.sorted { $0.timestamp < $1.timestamp }
        for index in 0 ..< (sortedRecords.count - 5) {
            let timeSpan = sortedRecords[index + 4].timestamp.timeIntervalSince(sortedRecords[index].timestamp)
            if timeSpan < 300 {  // 5 minutes
                patterns.append(.burstUsage)
                break
            }
        }

        // Detect expensive request clusters
        let expensiveRequests = records.filter { $0.cost > 0.04 }
        if expensiveRequests.count > records.count / 3 {
            patterns.append(.highCostPattern)
        }
    ...
    private func generateOptimizationRecommendations(_ records: [AIRequestRecord]) -> [OptimizationRecommendation] {
        ...
        let quickRequests = records.filter { $0.type == .quick }
        if quickRequests.count > 10 {
            recommendations.append(
                OptimizationRecommendation(
                    type: .routingOptimization,
                    description: "Route quick analyses to local processing",
                    estimatedSavings: Double(quickRequests.count) * 0.005 * 0.9,
                    impact: .medium
                ))
        }
        ...
        if nightRequests.count > records.count / 4 {
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    description: "Defer non-urgent requests to optimize costs",
                    estimatedSavings: Double(nightRequests.count) * 0.01,
                    impact: .low
                ))
        }
...
struct UsagePatternAnalysis {
    let timeOfDayDistribution: [Int: Double]
    let requestTypeFrequency: [AnalysisRequestType: Double]
    let costEffectiveness: Double
    let unusualPatterns: [UsageAnomalyPattern]
    let recommendations: [OptimizationRecommendation]
}

enum UsageAnomalyPattern: String {
    case burstUsage = "burst_usage"
    case highCostPattern = "high_cost_pattern"
    case unusualTiming = "unusual_timing"
    case lowSatisfaction = "low_satisfaction"
}
...
struct OptimizationRecommendation {
    ...
    enum OptimizationType: String {
        case routingOptimization = "routing"
        case schedulingOptimization = "scheduling"
        case batchOptimization = "batching"
        case cacheOptimization = "caching"
    }
