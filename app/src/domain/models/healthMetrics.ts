...
// RHR (Resting Heart Rate) データ
export interface RHRMetrics {
	value: number; // bpm
	baseline30d: number; // bpm (30日平均)
}

// RHRの算出プロパティ
export const getRHRDerivedMetrics = (
	rhr: RHRMetrics,
): {
	deviationPercent: number;
	status: RHRStatus;
	...
	const deviationPercent =
		rhr.baseline30d > 0
			? ((rhr.value - rhr.baseline30d) / rhr.baseline30d) * 100
			: 0;
	return {
		deviationPercent,
		status: getRHRStatus(deviationPercent),
	};
};

export type RHRStatus = "elevated" | "normal" | "slightlyLow" | "low";

// Note: For RHR, lower is generally better (opposite of HRV)
export const getRHRStatus = (deviationPercent: number): RHRStatus => {
	if (deviationPercent <= -10) return "low"; // Good - lower than baseline
	if (deviationPercent <= 5) return "normal";
	if (deviationPercent <= 15) return "slightlyLow"; // Actually slightly elevated
	return "elevated"; // High RHR - may indicate stress/fatigue
};
