...
### 1. サーカディアンリズム詳細画面（通常モード）

```
┌─────────────────────────────────────────┐
│ ← サーカディアンリズム                   │ ← ナビゲーションバー
├─────────────────────────────────────────┤
│                                         │
│ ┌─────────────────────────────────────┐ │
│ │         [24時間サークル図]          │ │ ← 24時間を円で表示
...
### 詳細画面の共通構造
- **ナビゲーションバー**: 戻るボタン + 画面タイトル
- **メインビジュアル**: 画面上部に主要データを大きく表示
- **セクション分け**: ─線で視覚的に区切り
- **縦スクロール**: 全体がスクロール可能

### セクション構成パターン
1. **メインデータエリア**: 現在値・大きな数値・視覚的表現
2. **トレンドエリア**: 週間グラフ・時系列データ
3. **詳細データエリア**: 数値の内訳・平均値
4. **説明・ヒントエリア**: 解釈・改善提案

### 24時間サークル図の構成
- **通常モード**: 1つの円で就寝・起床時刻を表示
- **トラベルモード**: 2つの時刻（体内時計 vs 理想時刻）を重ねて表示
- **視覚的区別**: ●実際 vs ○理想、太線 vs 点線
