...
┌─────────────────────────────────────┐
│ 12月11日（木）                      │ ← 日付表示
│ ☀️ 東京 12°C            ✈️[OFF]    │ ← 天気・トラベルモード
├─────────────────────────────────────┤
│                                     │
│ おはようございます、マサさん        │ ← 時間帯別挨拶
│                                     │
├─────────────────────────────────────┤
│ ┌─────────────────────────────────┐ │
│ │ 🌙 今日のアドバイス              │ │ ← アドバイスサマリーカード
│ │                                 │ │   (Phase 2で実装済み)
│ │ 昨夜は7時間の良質な睡眠が      │ │
│ │ 取れましたね。HRVは65msと      │ │
│ │ 安定しています。今日は集中      │ │
...
│ ┌─────────────────────────────────┐ │
│ │ 📅 今週のトライ                 │ │ ← このフェーズで実装
│ │ 「毎朝同じ時間に起きる習慣」     │ │   (月曜以外はコンパクト表示)
│ └─────────────────────────────────┘ │
│                                     │
├─────────────────────────────────────┤
│  [ホーム]  [コンディション]  [設定] │ ← タブバー(Phase 2で実装済み)
└─────────────────────────────────────┘
...
### メトリクスカードグリッド配置
- **配置**: 2×2グリッド（縦2行 × 横2列）
- **順序**: 左上→右上→左下→右下（回復→睡眠→エネルギー→ストレス）
- **カード間隔**: 中程度のマージン（ui-spec.mdの「中」サイズ）
- **各カードサイズ**: 等幅・等高

### トライカードのスタッキング
- **今日のトライ**: より目立つ表示（Accent Colorのアクセント）
- **今週のトライ**: 
  - 月曜日: 目立つ表示（大きめ + "NEW!" ラベル）
  - 火〜日曜: コンパクト表示（1行表示）

### 追加アドバイスの配置
- **位置**: 画面下部にフローティング表示（タブバーの上）
- **表示条件**: 13:00以降 or 18:00以降の初回起動時
- **インタラクション**: ×ボタンまたは上スワイプで閉じる
