...
import type { Mood, TodayMode } from "../domain/models";
import type { Alert, TopDiscovery } from "../domain/models/insight";
	...
	persist(
		(set, get) => ({
			...
			generateDailyInsight: async () => {
				...
				try {
						...
						const request = buildAdviceRequest({
							healthStore: {
								metrics: {
									sleep: healthStoreState.metrics.sleep
										? {
												durationMinutes:
													healthStoreState.metrics.sleep.durationMinutes,
												deepSleepMinutes: Math.round(
													healthStoreState.metrics.sleep.durationMinutes *
														healthStoreState.metrics.sleep.deepSleepRatio,
												),
												remSleepMinutes: Math.round(
													healthStoreState.metrics.sleep.durationMinutes *
														healthStoreState.metrics.sleep.remSleepRatio,
												),
											}
										: null,
							...
							userStore: {
								profile: userStoreState.profile
									? {
											goals: (userStoreState.profile as { goals?: unknown })
												.goals ?? ["better_sleep"],
											wakeUpTime:
												(userStoreState.profile as { wakeUpTime?: unknown })
													.wakeUpTime ?? "07:00",
											windDownTime:
												(userStoreState.profile as { windDownTime?: unknown })
													.windDownTime ?? "23:00",
										}
									: null,
							},
					...
					if (!response.success) {
						throw new Error(
							response.error?.message || "アドバイスの生成に失敗しました",
						);
					}
			...
			initializeWithMockData: () => {
				if (DATA_SOURCE_CONFIG.USE_MOCK_DATA) {
					const { setDailyInsight } = get();
					setDailyInsight({
						todayInsight: MOCK_AI_RESPONSE.todayInsight,
						todayOneThing: MOCK_AI_RESPONSE.todayOneThing,
						relatedInsight: MOCK_AI_RESPONSE.relatedInsight,
					});
				}
			},
		}),
		{
			...
			partialize: (state) => ({
				...
				todayInsight: state.todayInsight,
				todayOneThing: state.todayOneThing,
				relatedInsight: state.relatedInsight,
				lastInsightUpdate: state.lastInsightUpdate,
			}),
		},
	),
);
...
export const selectTodayOneThing = (
	state: InsightState,
): TodayOneThing | null => state.todayOneThing;

export const selectRelatedInsight = (
	state: InsightState,
): RelatedInsight | null => state.relatedInsight;
