...
export const calculateSleepScore = (input: SleepScoreInput): number => {
	...
	const deepScore = scoreRange(
		deepRatio,
		SLEEP_STAGE_THRESHOLDS.deep.min,
		SLEEP_STAGE_THRESHOLDS.deep.max,
	);
	const remScore = scoreRange(
		remRatio,
		SLEEP_STAGE_THRESHOLDS.rem.min,
		SLEEP_STAGE_THRESHOLDS.rem.max,
	);
...
export const calculateRhythmScore = (input: RhythmScoreInput): number => {
	const bedtimeScore = getConsistencyScore(input.bedtimeStddevMinutes);
	const wakeScore = getConsistencyScore(input.wakeTimeStddevMinutes);
