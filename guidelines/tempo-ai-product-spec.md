...
- 🇺🇸 **英語**（デフォルト）
  - 対象市場: 北米、ヨーロッパ、アジア（非日本語圏）
  - App Store: 米国、英国、カナダ、オーストラリアなど
...
デバイス言語を検出
  ↓
・iPhoneが日本語設定 → アプリも日本語で起動
・iPhoneが英語設定 → アプリも英語で起動
```

#### 2. 言語切り替え
プロフィール画面から手動で変更可能：
- 日本語 ⇄ English
- 変更後、全てのテキストとAIアドバイスが選択言語で表示

#### 3. 多言語対応箇所

**完全対応:**
- ✅ アプリ内の全UI（ボタン、ラベル、説明文）
- ✅ オンボーディング画面
- ✅ プロフィール設定画面
- ✅ 通知メッセージ
- ✅ **AIが生成するアドバイス（最重要）**
...
**例: 英語でアドバイス生成**
```
Today's Theme: Blood Sugar Stability Day

Breakfast:
1 cup of oatmeal with...
(Reason: Your sleep was 6.5 hours last night...)

Exercise:
Light walking for 30 minutes
(Reason: Your HRV is lower than usual...)
```
...
#### 展開優先順位
1. **Phase 1**: 英語 + 日本語（北米・日本）
2. **Phase 4**: スペイン語（大規模市場）
3. **Phase 5**: フランス語、ドイツ語（ヨーロッパ）
4. **Phase 6**: 中国語、韓国語（アジア拡大）
