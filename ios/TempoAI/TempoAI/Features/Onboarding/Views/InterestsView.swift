        ...
        Text("次へ")
          ...
          .background(
            RoundedRectangle(cornerRadius: 16)
              .fill(isSelectionValid ? Color.tempoSoftCoral : Color.tempoLightGray)
          )
      }
  ...
  var body: some View {
    Button(action: onTap) {
      VStack(spacing: 12) {
        ...
        .frame(minHeight: 60) // テキストエリアの最小高さを確保
      }
      .frame(maxWidth: .infinity)
      .frame(height: 160) // カード全体の高さを少し大きく
      ...
      .background(
        RoundedRectangle(cornerRadius: 16)
          ...
          .stroke(
            isSelected ? Color.colorForInterest(interest) : Color.tempoLightGray.opacity(0.5),
