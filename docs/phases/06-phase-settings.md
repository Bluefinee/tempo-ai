...
```
┌─────────────────────────────────────┐
│ [←] 関心ごとタグ編集    [保存]      │ ← ナビゲーションバー
├─────────────────────────────────────┤
│                                     │
│ あなたの関心ごとを選んでください    │ ← 説明文
│ （1〜3個）                          │
│                                     │
│ ┌─────────────┬─────────────────┐ │
│ │      ✓      │                    │ │ ← 2×3グリッド
...
### 設定トップ画面の構造
- **セクション分け**: 「アカウント」と「その他」の2セクション
- **項目表示**: 各項目に現在値を表示（プロフィールは要約、タグは選択済みタグ）
- **ナビゲーション**: 右端のchevron（>）で遷移可能であることを明示

### フォーム画面の共通原則
- **保存ボタン**: ナビゲーションバー右上に配置
- **フィールド配置**: ラベル + 入力フィールドの縦スタック
- **セクション区切り**: 基本情報と任意情報を視覚的に分離
- **フィールドサイズ**: タップしやすい十分な高さ

### タグ選択の配置
- **グリッドレイアウト**: 2列×3行の6タグ配置
- **選択状態**: ✓マークで明確に表示
- **選択制限**: 1〜3個選択、視覚的フィードバックで範囲を明示

### 入力フィールドの種類
- **テキスト入力**: ニックネーム
- **数値入力**: 年齢、体重、身長
- **選択式**: 性別、職業、生活習慣（ピッカー/ドロップダウン）
- **タグ選択**: 関心ごと（マルチセレクト）
