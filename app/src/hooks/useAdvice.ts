...
import type { UserGoal, WeatherData } from "../api/types";
...
export const useAdvice = (): UseAdviceReturn => {
			...
			sleep: state.metrics.sleep
				? {
						...
						deepSleepMinutes: Math.round(
							state.metrics.sleep.durationMinutes *
								state.metrics.sleep.deepSleepRatio,
						),
						remSleepMinutes: Math.round(
							state.metrics.sleep.durationMinutes *
								state.metrics.sleep.remSleepRatio,
						),
					}
				: null,
	...
	const userStore = useUserStore((state) => ({
		profile: state.profile
			? {
					goals: (state.profile as { goals?: UserGoal[] }).goals ?? [
						"better_sleep",
					],
					wakeUpTime:
						(state.profile as { wakeUpTime?: string }).wakeUpTime ?? "07:00",
					windDownTime:
						(state.profile as { windDownTime?: string }).windDownTime ??
						"23:00",
				}
			: null,
	}));
	...
	const fetchAdvice = useCallback(
		async (weather: WeatherData): Promise<void> => {
			setIsLoading(true);
			setError(null);
