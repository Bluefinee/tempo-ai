...
import { RhythmInteractiveChart } from "../../src/components";
import { DEFAULT_ENVIRONMENT_DATA } from "../../src/constants/environmentConstants";
import { getPressureTrendIcon } from "../../src/domain/models/weather";
import { DEFAULT_RHYTHM_DATA } from "../../src/domain/services/rhythmCalculator";
import { getLocale, t } from "../../src/i18n";
import {
	useEnvironmentData,
	useRhythmChartData,
	useUpcomingWindows,
} from "../../src/stores/healthStore/selectors";
...
const RhythmScreen = (): React.ReactElement => {
	...
	const dateString = formatDate(now, getLocale());

	// healthStoreからデータを取得（フォールバック付き）
	const rhythmChartData = useRhythmChartData();
	...
	const peakFocusTime =
		peakFocus?.timeRange ?? t("screen.rhythm.phases.peakEnergy.time");
	const melatoninTime =
		melatoninWindow?.timeRange ??
