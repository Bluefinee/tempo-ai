...
struct TrendAnalyzer {
    ...
    private static func analyzeHeartRateTrends(_ heartRateMetrics: HeartRateMetrics?) -> [HealthTrend] {
        ...
        if let variabilityScore = heartRate.variabilityScore {
            let direction: TrendDirection = variabilityScore > 80 ? .improving : 
                                           variabilityScore < 60 ? .declining : .stable
    ...
    private static func analyzeSleepTrends(_ sleepData: EnhancedSleepData) -> [HealthTrend] {
        ...
        trends.append(HealthTrend(
            ...
            timeframe: .week,
            significance: significance,
            description: "睡眠時間は\(String(format: "%.1f", sleepHours))時間で\(direction == .stable ? "適切" : direction == .improving ? "改善傾向" : "注意が必要")です"
        ))
        
        // Sleep efficiency trend
        let efficiencyDirection: TrendDirection = sleepData.sleepEfficiency >= 0.85 ? .improving :
                                                  sleepData.sleepEfficiency < 0.75 ? .declining : .stable
        ...
        trends.append(HealthTrend(
            ...
            description: "睡眠効率は\(String(format: "%.0f", sleepData.sleepEfficiency * 100))%で\(efficiencyDirection == .improving ? "良好" : efficiencyDirection == .stable ? "標準的" : "改善の余地があります")"
        ))
        ...
        if let deepSleep = sleepData.deepSleep {
            ...
            let deepSleepDirection: TrendDirection = deepSleepPercentage >= 20 ? .improving :
                                                     deepSleepPercentage < 15 ? .declining : .stable
    ...
    private static func analyzeActivityTrends(_ activityData: EnhancedActivityData) -> [HealthTrend] {
        ...
        trends.append(HealthTrend(
            ...
            description: "1日の歩数は\(activityData.steps)歩で\(stepsDirection == .improving ? "目標を達成" : stepsDirection == .stable ? "標準的" : "増加が推奨")されます"
        ))
        
        // Exercise time trend
        let exerciseDirection: TrendDirection = activityData.exerciseTime >= 30 ? .improving :
                                                activityData.exerciseTime < 15 ? .declining : .stable
        ...
        trends.append(HealthTrend(
            ...
            description: "運動時間は\(activityData.exerciseTime)分で\(exerciseDirection == .improving ? "十分" : exerciseDirection == .stable ? "標準的" : "増加が推奨")です"
        ))
        
        // Stand hours trend
        let standDirection: TrendDirection = activityData.standHours >= 10 ? .improving :
                                             activityData.standHours < 6 ? .declining : .stable
        ...
        trends.append(HealthTrend(
            ...
            description: "立位時間は\(activityData.standHours)時間で\(standDirection == .improving ? "良好" : standDirection == .stable ? "標準的" : "増加が推奨")です"
        ))
        ...
        trends.append(HealthTrend(
            ...
            timeframe: .week,
            significance: 0.7,
            description: "総合活動レベルは\(activityLevel == .extremelyActive || activityLevel == .veryActive ? "非常に良好" : activityLevel == .moderatelyActive ? "標準的" : "改善の余地があります")です"
        ))
    ...
    private static func analyzeOverallTrends(_ data: ComprehensiveHealthData) -> [HealthTrend] {
        ...
        trends.append(HealthTrend(
            ...
            description: "総合的な健康状態は\(overallDirection == .improving ? "良好な方向" : overallDirection == .stable ? "安定" : "注意が必要な状況")にあります"
        ))
        
        // Recovery trend based on HRV and sleep
        if let hrv = data.vitalSigns.heartRateVariability {
            let recoveryScore = hrv.recoveryScore
            let recoveryDirection: TrendDirection = recoveryScore >= 70 ? .improving :
                                                    recoveryScore < 50 ? .declining : .stable
            ...
            trends.append(HealthTrend(
                ...
                timeframe: .week,
                significance: 0.8,
                description: "体の回復状態は\(recoveryDirection == .improving ? "良好" : recoveryDirection == .stable ? "標準的" : "注意が必要")です"
            ))
        }
